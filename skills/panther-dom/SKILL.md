...
Path: source-of-truth is the personalities source repo, not the plugin cache. On the user's dev machine that's `/home/laragana/workspace/personalities/skills/panther-dom/memory/`. If reading fails (e.g. fresh machine where `sync-brain-files.sh` hasn't bootstrapped the brain yet), fall back gracefully — read what's there, write what's writable. The brain files themselves are gitignored, so cross-machine memory sync is manual via the `species/<name>/brain-starter.md` mechanism (or its dom equivalent).
